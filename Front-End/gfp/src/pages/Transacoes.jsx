...
export default function Transacoes(){
    ...
    const buscarDadosAPI = async () => {
        ...
        const formatarData = (data) => {
            ...
            return dataFormatada.toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit', year: '2-digit' });
        }
        ...
        const exibirItemLista = (item) => {
            ...
            return(
                <div key={item.id_transacao} className="flex items-center bg-gradient-to-r from-white-100 via-purple-100 to-gray-100 shadow-lg rounded-xl p-4 mb-4 transition-transform hover:scale-101">
                    ...
                    <div className="flex-1">
                        <p className="text-lg font-bold text-gray-800 mb-1">{item.descricao}</p>
                        <div className="flex justify-between items-center">
                            <div>
                                ...
                                {/* status */}
                                <div className={`flex items-center gap-1 text-sm font-medium ${status.cor}`}>
                                    {status.icone}
                            ...
                            <div className="flex flex-col items-end">
                                {/* valor */}
                                <span className={`font-bold text-lg ${item.tipo_transacao == 'SAIDA' ? 'text-red-500' : 'text-green-500'}`}>
                                    {item.tipo_transacao === 'SAIDA' ? '-' : '+'} R$ {parseFloat(item.valor).toFixed(2)}
                                </span>
                                <div className="flex space-x-2 mt-2">
                                    {
                                        !item.data_pagamento && <button title="Quitar" className="bg-white border border-gray-300 rounded-full p-2 hover:bg-blue-200 transition-colors" onClick={() => botaoQuitar(item.id_transacao)}>
                                            <MdDone className="text-blue-600 text-xl" />
                                            </button>
                                    }
                                    
                                    <button title="Excluir" className="bg-white border border-gray-300 rounded-full p-2 hover:bg-red-200 transition-colors" onClick={() => botaoExcluir(item.id_transacao)}>
